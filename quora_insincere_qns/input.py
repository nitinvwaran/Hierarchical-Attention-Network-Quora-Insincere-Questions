...
def get_train_df_glove_dict(train_file, glove_file,mmap_loc, is_training = True, reload_mmap=False):
    ...
    if (is_training):
        ...
        X_train_0_sample = X_train_0.sample(n=1200000)
        X_train_1_sample = X_train_1.sample(n=80000)
...
def process_questions(qn_df, glove_dict, mmap, is_training = True):
    ...
    for index, item in qn_df.iterrows():
        ...
        for y in qn_ls_word:
            ...
            #tmp0 = [x for x in y if x not in stop_words]
...
def build_graph(max_sentence_len):
    ...
    with tf.variable_scope('fc_layers'):
        ...
        fc1_size = 64
        fc1 = tf.contrib.layers.fully_connected(outputs_sent, fc1_size,
                                                             activation_fn=tf.nn.relu)
        fc2_size = 64
